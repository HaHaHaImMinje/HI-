		...
		<div class="col-md-8">
			...
			<div class="list-group">
				<c:forEach var="i" items="${boardList}" varStatus="cnt">
					<a href="boardInfo.do?no=${i.no}&job=info" class="list-group-item">
						<div class="board-title">
							<c:forEach var="j" begin="0" end="${i.lev}">
								&nbsp;&nbsp;&nbsp;&nbsp;
							</c:forEach>
							<span class="board-category"> <img
								src='/springWeb/upload/${i.fileName}' width="30" height="30">
								[ALGORITHM]
							</span> ${i.title} <span class="badge"> ${i.no ne i.ref?'REPLY':'New'}</span>

						</div>
						<div class="board-meta"
							style="font-weight: 400; font-size: 1.2rem; color: #404040">
							<p>
								<i class="glyphicon glyphicon-user">${i.author} 님</i> <i
									class="glyphicon glyphicon-comment">${i.reply }</i>  <i
									class="glyphicon glyphicon-ok">${i.hit }</i>  <i
									class="glyphicon glyphicon-time">${i.regdate }</i> 
							</p>
						</div>
					</a>
				</c:forEach>
